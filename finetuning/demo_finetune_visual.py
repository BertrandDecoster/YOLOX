...
def make_parser():
    ...
    parser.add_argument(
        "--path", 
        default="./datasets/trashcan_test/train2017/frame_0000.jpg", 
    ...
    parser.add_argument(
        "-c", 
        "--ckpt", 
        default="extended_overfit_model.pth", 
        type=str, 
...
def main():
    ...
    for idx, image_path in enumerate(image_paths):
        ...
        result_image, detections = predictor.visual(
            outputs[0] if outputs[0] is not None else None, 
            img_info
        )
    ...
    logger.info("\n" + "-"*60)
    logger.info(f"OVERALL: {total_detections} detections across {len(all_results)} images")
    if total_by_class:
